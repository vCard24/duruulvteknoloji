<?php
/**
 * Fiyat teklifi formu — HTML e-posta (Hostinger SMTP veya PHP mail)
 */
...
if (is_array($formData)) {
    ...
    $html = yck_build_quote_email_html($formData);
} else {
    $name  = trim((string) ($payload['name'] ?? ''));
    $email = trim((string) ($payload['email'] ?? ''));
    $phone = trim((string) ($payload['phone'] ?? ''));
    if ($html !== '') {
        $html = yck_strip_data_urls_from_html($html);
    }
}
...
if (strlen($html) > 800000) {
    yck_json_response(413, ['ok' => false, 'error' => 'E-posta içeriği çok büyük.']);
}
...
$subject = 'Yiğit Çelik Kapı — Fiyat teklifi: ' . $subjectName;
...
if (!$sent) {
    ...
    yck_json_response(500, [
        'ok'    => false,
        'error' => 'E-posta gönderilemedi. api/config.php içinde SMTP ayarlarını kontrol edin.',
    ]);
}
